...
function print_path_for_development()
{
	global $db;
	// && $_SERVER['hostname'] 
	if ( $db->showErrorOnLocal == TRUE && $_SERVER['SERVER_NAME'] == 'localhost' ) {
		$result = true;
	} else {
		$result = false;
	}
...
//FOR LOCALHOST
//str_replace("wamp/www","Development projects",$rootS);
$folderChangeTO = 'E:/Development projects'; //development folder name that use for alias
$folderChangeFrom   =   'E:/xampp/htdocs';
$aliasFolder = 'projects';

// $folderChangeTO = ''; //development folder name that use for alias
// $folderChangeFrom   =   '';
// $aliasFolder = '';
...
if($_SERVER['HTTP_HOST']=='localhost' || $_SERVER['REMOTE_ADDR'] == '192.168.10.4'){
$rootS      =   $_SERVER['DOCUMENT_ROOT'];
$rootS      =   str_replace($folderChangeFrom,$folderChangeTO,$rootS);
define('DOCUMENT_ROOT',	$rootS);
}else{
...
if($_SERVER['HTTP_HOST']=='localhost' || $_SERVER['REMOTE_ADDR'] == '192.168.10.4'){
    $image  = str_replace($aliasFolder,"",$image);
}
...
if (!file_exists($docRoot . $image))
{
    ...
    if($_SERVER['HTTP_HOST']=='localhost'){
        echo "<br>go in src/image.php and change folder name to replace";
    }
	exit();
}
...
if (!isset($_GET['nocache']) && file_exists($resized))
{
	...
	if($imageModified < $thumbModified && $data != "") {
