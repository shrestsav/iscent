...
class product_function extends object_class{
    ...
    public function getProductName($pid){
        ...
        $data   =   $this->dbF->getRow($sql);
        if($this->dbF->rowCount>0){
            $name   =   unserialize($data['prodet_name']);
            return  $name[$defaultLang];
        }
        return false;
    }
...
//get sale offer
    ...
    public function totalOrderSubmit($userId=false,$invoiceStatus=false){
        ...
        if($invoiceStatus===false || $invoiceStatus==''){
            ...
            if(is_numeric($invoiceStatus)){
                $invoiceStatus = " =  '$invoiceStatus'";
            }
            $invoiceStatus =   " $and invoice_status $invoiceStatus";
        }

        $sql    =   "SELECT count(order_invoice_pk) as qty FROM order_invoice WHERE $userId $invoiceStatus";
        $sql    =   trim($sql);
        $sql    =   trim($sql,'WHERE');
        $data   =   $this->dbF->getRow($sql);
        if($this->dbF->rowCount>0){
