...
function addListItem() {
    ...
    var pName = $(".invoice_product_id").attr('data-val') +" -- "+ $(".invoice_product_scale").attr('data-val') +" -- "+ $(".invoice_product_color").attr('data-val');
    ...
    if (document.getElementById("tr_"+trpid)) {
        ...
        var item = "<tr id='tr_"+trpid+ "' data-id='"+trpid+"'>"+
            ...
            "<td>"+pName+
                ...
                "<input type='hidden' name='pcolor_"+trpid+"' value='"+pColorId+"' />"+

                "<input type='hidden' name='pweight_"+trpid+"' value='"+pWeight+"' class = 'product_weight' />"+

                "<input type='hidden' name='pInter_"+trpid+"' value='"+interShipping+"' class='interShipping'/>" +

                "<input type='hidden' name='pQty_"+trpid+"' value='"+qty+"' class='addedQty'/>" +

                "<input type='hidden' name='pPrice_"+trpid+"' value='"+price+"' />" +

                "<input type='hidden' name='pDiscount_"+trpid+"' value='"+discount+"' class='addedDiscount'/>" +
            ...
            "<td> ("+qty+"*"+price+")-"+discount+"="+totalPrice+" "+priceCode +"</td>"+
...
function formSubmit(){
    ...
    if ( $('#vendorProdcutList tr').length <= 0 ) {
        ...
        notification('<?php echo _js($_e["Product Error"]); ?>','<?php echo _js($_e["You not select any product for order."]); ?>',"btn-warning");
...
function finalFormSubmit(){
    ...
    if ( $('#vendorProdcutList tr').length <= 0 ) {
        ...
        notification('<?php echo _js(_uc($_e["Product Error"])); ?>','<?php echo _js($_e["You not select any product for order."]); ?>',"btn-warning");
...
delOrderInvoice = function(ths){
    ...
    if(secure_delete()){
        ...
        $.ajax({
            ...
            url: 'order/order_ajax.php?page=delOrder&id='+id,
...
$('#SelectProduct ul li').click(function(){
    var nm = $(this).find('h4').text();
    var pid = $(this).find('.h_pid').val();
    var pvd = $(this).find('.h_validity').val();
    var ppm = $(this).find('.h_pm').val();
    var ppr = $(this).find('.h_price').val();
    $(this).parent().prev('span').text(nm);
    $('.s_valid').val(pvd);
    $('.s_price').val(ppr);
    $('.s_payment').val(ppm);
    $('.s_productid').val(pid);
...
function chk(){
    var pn = $('#SelectProduct').find('span').text();
    if(pn=='Select Product'){
        jAlertifyAlert('<?php echo _js($_e['Please Select Product']); ?>');
        return false;
    }
}
