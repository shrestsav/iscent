...
$.extend(true, $.fn.dataTable.defaults, {
    ...
    "fnInitComplete": function (oSettings, json) {
        ...
        if (currentId) {
            ...
            //form-group was use here
            thisLength.addClass('col-xs-6 ');
            thisLengthLabel.addClass('control-label ').attr('for', currentId + '_length_select').css({
                'text-align': 'left',
                'font-weight': 'normal',
                'padding-right': '10px',
                'text-transform': 'capitalize'
            });
            thisLengthSelect.addClass('form-control').attr('id', currentId + '_length_select').css({
                'display': 'inline-block'
            });
            /*//thisLengthSelect.prependTo(thisLength).wrap('<span class=" " />');
            // Re-arrange the search input for a form-horizontal layout*/
            //form-group was use here
            thisFilter.addClass('pull-right form-group has-feedback').css({
                'margin-right':'15px'
            });
            thisFilterLabel.addClass('control-label ').attr('for', currentId + '_filter_input').css({
                'text-align': 'left',
                'font-weight': 'normal',
                'text-transform': 'capitalize'
            });
            thisFilterInput.addClass('form-control').attr('id', currentId + '_filter_input');
          /*  thisFilterInput.appendTo(thisFilter).wrap('<span class=" " />');*/
            thisFilterInput.after('<span class="glyphicon glyphicon-search form-control-feedback" />');
        }
    }
});
...
if ($.fn.DataTable.TableTools) {
    ...
    $.extend(true, $.fn.DataTable.TableTools.classes, {
        "container": "DTTT btn-group pull-right container-fluid",
        "buttons": {
